...
#------------------------------------------------------------------------
#
#   $Id: Template.pod,v 1.36 2000/02/01 12:34:57 abw Exp $
#
#========================================================================
...
The 'join' method can be called on a list to concatenate all values
into a single string.  Values will be separated by a space or an
alternate string can be passed as a parameter.
...
Specify the <:status> tag as a parameter to C<use Template> to 
import the STATUS_* definitions.
...
The plugin is derived from the Template::Plugin module which provides
a general facility for wrapping external objects.  If a method returns
more than one value (such as the CGI method checkbox_group(), above)
then the results will be concatenated into a single string, each
separated by a space.  The joint() method can be used to change the
joining string.

    [% USE CGI %]
    [% CGI.joint("\n") %]  # multi-values now separated by newline

A method may have the '_as_list' suffix added to have the result(s)
returned as a list reference.

    [% checklist = CGI.checkbox_group_as_list(name   => 'names', 
					      values => [ 'foo' 'bar' ])
    %]
...
block.  The Template::Filters module defines the 'html',
'format', 'redirect' and 'into' filters.  The 'html' filter converts
the characters '<', '>' and '&' to '&lt;', '&gt;' and '&amp',
respectively, protecting them from being interpreted as representing
HTML tags or entities.  The 'format' filter takes a format string as a
parameter (as per printf()) and formats each line of text accordingly.
...
This is version 1.03 of the Template Toolkit.  
