...
#------------------------------------------------------------------------
#
#   $Id: Template.pod,v 2.3 2000/09/12 15:25:17 abw Exp $
#
#========================================================================
...
=head1  SUMMARY OF TEMPLATE DIRECTIVES
...
error() method.  See also L<CONFIGURATION OPTIONS> which describes how
...
exception objects).  The type() and info() methods can be used to
...
really are.  Thus, you'll generally see:
...
appropriate END directive.  Blocks may be nested indefinately.  The
...
variable values.  The following methods can be called against any
scalar value:
...
reference as the first item.  SCALAR_OPS are subroutines that accept
a scalar value as the first item.  Any other arguments specified when 
the method is called will be passed to the subroutine.
...
as an alias for '%' for backwards compatability with version 1.
...
You can also use anonymous BLOCK to capture the output of 
a template fragment, should you need to:
...
iteration, and so on.  See L<Template::Iterator> for further details.
...
Toolkit or you can define your own via the FILTERS configuration 
option.

The 'html' filter, for example, escapes the 'E<lt>', 'E<gt>' and
'&' characters to prevent them from being interpreted as HTML tags
or entity reference.
...
See L<TEMPLATE TOOLKIT FILTERS> for a complete list of available filters,
...
See L<TEMPLATE TOOLKIT PLUGINS> for more information on the plugins
...
of the 'DBI' type.   The other specific exception caught here is of the 
'file' type.
...
variable so these errors file can quite happily access the 'error'
...
storing that many compiled templates.  When a new template is loaded and 
compiled and the cache is full (i.e. the number of entries == CACHE_SIZE), 
the least recently used compiled template is discarded to make room for the
new one.
...
use (e.g. cache persistance).  The COMPILE_EXT option may be
...
The FILTERS option can be used to specify additional filters which can
then be used with the FILTER directive.  These are added to the
standard filters which are available by default.  Filters specified
via this option will mask standard filters of the same name.

The FILTERS should be specified as a reference to a hash array with
each key representing the name of a filter.  The corresponding value
should contain a reference to an array containing a subroutine
reference and a flag which indicates if the filter is static (0) or
dynamic (1).  A filter may also be specified as a subroutine reference
by itself and is assumed to be static.
...
The '|' character is an alias for FILTER, allowing the previous example
to be written as:
...
It is an unfortunate circumstance that this filter cannot simply be called
'perl'.  This is due to the clash with the reserved word of that name 
(i.e. PERL blocks).
