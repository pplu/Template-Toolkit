...
B<NOTE:> This documentation is incomplete and may be incorrect in
places.  Also note that the Splash! library is subject to evolve
in future versions and may be subject to change.
...
The Splash! library defines a small selection of named colours.  Each
different colour scheme requires a number of small images (primarily
rounded corners) which are used to build the user interface
components.  These are located in the 'images/splash' directory
beneath the installation directory.  More colour schemes will likely
become available in the near future (but probably distributed
separately).  Currently the fully supported colours are:

    aqua, bud, leaf, lilac, marine, mauve and mint

The 'rgb' hash defined by in the html/rgb template is imported as the
'splash.rgb' hash.  You can use some of the additional colours to
specify background colours, etc, although they're not all fully 
supported for general use (i.e. there currently aren't the required
images to go with them for the rounded corners).  e.g.
